...
```
  ...
  So the first demographics we'll take a quick peak at is at age a race. First we'll look at age at a histogram, and get used to it, we're going to be looking at age a lot in these plots. But overall age is right skewed, with only one real outlier at around 42. Alex G is about 26-27 at the time of me writing this and has been writing music since his early teenage years, so the demographics of teens and 20-somethings is unsurprising.
...
#Age
age_plot <- ggplot(dat, aes(age)) +
  geom_histogram(breaks=seq(15, 45, by=1),
...
First we gotta do some MASSIVE data cleaning, I really should not of left this open for people to put whatever they want, look at all the unique comments, as of writing there is 37. So we gotta fine a way to generalize these. That said, some are very interesting
...
Making the plot now simplified
```{r}

ggplot(dat_disc, aes(x = discover_method)) +
  geom_bar()  +
  coord_flip() +
...
## Other thanks in order of commenting on the original post
 ...
 + Fabiana O.: Suggested questions on race and queerness questions

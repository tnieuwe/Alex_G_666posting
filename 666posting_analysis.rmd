...
#plot
I should see if it is possible for me to clean up this histogram
```{r}
##Two histograms
...
One of the other plots I found interesting in my analysis that set this off was looking at the growth of Alex G's fanbase through his career. However, unlike my initial making of this plot based on people just commenting on a facebook post, this data is a little... odd. 
```{r}
##Second histogram
...
So below is the original figure.
 ...
 And compared to this figure our new one is definitely different. First of all we we have much more of a normal distribution besides the person... who first heard Alex G in '07. Frankly I'm a little dubious and would be interested in hearing that story. The individual did not identifty which state they live in, so I though my worries would be assuaged by hearing they lived in PA, but alas, I have no idea. The other interesting aspect is our first non '07 fan comes in around 2011, after Race, an arguably seminal Alex G album. I would frankly be expecting a larger boost with Race, and the previous plot I made did have individuals listening around the time of Race, admittedly few, but more than none.
 Also compared to the previous plot we have a lot more 2018-202 fans, but I think that's primarily because the original data was pulled from late March/early April 2020, and this current survey has been rolling open the entire year, so that in not unsuspected. But on the topic of the actual data, the data is for the most part normal, which is more indicative of the sample we're pulling from as it should just continue increasing with each album most likely. However it is interesting that 2015 and the release of Beach Music is his largest jump in popularity, as it is also the same year he signed with Domino Records.
...
I just like cumulative distribution plots, its all the same data.
```{r}
...
# Make a map of the world
 ...
 So Alex G, for those not in the know, lives in Philly, however like most succesful artists people want to hear him elsewhere and he tours pretty regularly. So I thought it would be intersting to see where Alex G fans are all over the world!
...
The first graph is a scatterplot of the political information you gave, shapes are based on the gender identity of the individuals who participated. Some plots are off the compass, that’s because of the jitter I placed on the data to prevent too many points from overlapping. I also marked the median and mean political identity. 

The second graph is another scatterplot, but using the non-jittered values and instead using the size of the points to indicate how many individuals identify with said political identity. 

I have also added the image I used in the survey for the political compass.

I am not surprised by these results, based on the general demographics of those who answered the survey. Also Alex G is a supporter of Bernie Sanders so I assume he would most likely fall in the general area most people did.
...
Political compass with age as a variable, I had to remove the individual over 40 because that threw the scale off.
```{r}
...
Okay here I'm about to go on a visualization tirade. So I'm sure you're used to venn diagrams, but have probably not hear of Upset plots. So venn diagrams are the classic way to represent count crossover between multiple groups, but when groups are large this generally reduces to circles with numbers inside of them. This doesn't do the scale of the differences proper justice, and people, in general, have a better time understanding the magnitude of difference when there is a visualization, especially comparing sizes between ideas. As a result a new plot has been formed called upset plots.
...
#I'm trying to avoid making all lower than upper
...
#Favorite musician
avg_musician <- names(sort(table(musician_dat$favorite_musician),decreasing=TRUE)[1])
#Second fave musician
second_musician <- names(sort(table(musician_dat$sec_musician),decreasing=TRUE)[1])
#Non Alex musician
second_2_musician <- names(sort(table(musician_dat$sec_musician),decreasing=TRUE)[2])
####NEED TO DO ART
avg_art <- (sum_art %>% arrange(desc(Freq)))[1,1]
##How often they go to concerts
avg_concert_go <- names(sort(table(dat$concerts),decreasing=TRUE)[1])
# Weed
avg_weed <- names(sort(table(dat$weed_use),decreasing=TRUE)[1])
#Alcohol 
avg_alcohol <- names(sort(table(dat$alc_use),decreasing=TRUE)[1])

#tolower()
```

So around `r mean(mdy_hm(dat$Timestamp))` a `r avg_race` `r avg_orientation` `r avg_gender` who is `r avg_age` years old decided to fill out the survey. They live in `r avg_country`, specifically in `r avg_state` and when asked if they prefer multiple partners they generally say `r avg_poly`. Their favorite musician is`r avg_musician`, when you ask them their second favorite musician they'll emphasize how much they like `r second_musician`, before balking and saying `r second_2_musician`. They have a lot of opinions about `r avg_musician`, their favorite song is `r avg_fave_song`, their favorite music video is `r avg_music_vid`, and their favorite album is `r avg_fave_album`. That said, they when asked about what their favorite fan compilation is they reply, `r avg_unreleased`, which is probably because they listen to most of their music through `r avg_music_source` which is somewhat difficult to get non-official tracks on.

Nevertheless they're still a big `r avg_musician` fan, they started listening to him when they were `r avg_start_listen` years old, and have been to `r avg_alex_conc` so far. When ask what they like to do in their spare time they have a couple hobbies. They like to play the `r avg_instrument`, but when asked if they're in a band, they'd say `r tolower(avg_band)`. They also use `r tolower(avg_art)` to express themselves, and when it comes to concerts they go `r tolower(avg_concert_go)`. When it comes to weed they would say "`r avg_weed`" and with alcohol they'd say they'd drink "`r avg_alcohol`".

# Special Thanks!

## Modmin Team of 666posting
In order Facebook decided:
...
## Other thanks in order of commenting on the original post
 ...
 + Abigail C.: For testing the survey and giving improvements

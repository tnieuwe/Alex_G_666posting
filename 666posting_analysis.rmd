...
Another plot I made, using the aforementioned "year learning of Alex G" data I extrapolated is a diagram of his growth. However, unlike my initial making of this plot based on people just commenting on a Facebook post, this data is a little... odd. 
```{r}
##Second histogram
...
This data I'm gonna use to center text in the states, it's basically each state’s central longitude and latitude.
```{r}
...
```
So the analysis I decided to do here was where is Alex's fan base the largest (when controlling for population size) in the United States. I pulled this region data from [kaggle](https://www.kaggle.com/omer2040/usa-states-to-region) and will use it to test our hypothesis. "Our hypothesis?" you say? Yes, obviously it is the [East Coast which has the most representation, if it’s not, all hope will be lost](https://www.youtube.com/watch?v=VgRYMNiePus). 
...
ggplot(genre_fave, aes(x = factor(Var1, rev(genre_fave$Var1)), y = Freq)) +
  ...
  ylab("Count") +
  labs(title ="Top Ten genreond Favorite Non-Alex G Artists")  + theme_classic()
```
...
Is there a secret formula to Alex G's top hits? Maybe we can use this data to figure it out! Remember though, this will be excluding non-officially released tracks.
...
## Birthday/Zodiacs
	...
	Gonna be honest, I am under qualified to say anything about what zodiac signs mean, so I'm going to leave these results up to your interpretation. That said, one thing I do know is these signs break up into the four classic elements, fire, water, air, and earth which are included in my analysis.
	
	
	So to generate the population data I pulled CDC birth records from 2000-2014 as an estimate. There appears to be a relatively uniform distribution of signs, and similarly elements in the general population. 
...
drug_table <- table(dat_drugs$weed_laws, dat_drugs$weed_use)
...
So it still appears that the biggest determinant of seeing Alex G concerts is if you live near Philly, which makes sense as, before Alex was touring most of his shows were in that area (and people could've included Skin Cells concerts as Alex G concerts). That said, if you remove the outliers, in the dataset, living near Philly is no longer a strong predictor on how many Alex G concerts you've seen, instead, living in a city trends towards significance.
...
## Other thanks in order of commenting on the original post
 ...
 + Abigail C.: For testing the survey and giving constructive critiques

## One special thanks

Graham aka Grumpus for talking to me about Alex G back when I knew diddly squat besides [Mary](https://www.youtube.com/watch?v=FR31HVr2_LU&ab_channel=PanfisWater). Thank you for introducing me to one of my favorite artists of all time. 

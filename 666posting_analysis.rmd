...
This is an analysis I completed on survey data I collected from a
Facebook group known as (Sandy) Alex G 666posting. For those of you that
don't know, Alex G is a prolific indie artist who has been putting out
music since at least 2010. He is now a well known musician in the indie
scene, but what makes him such an interesting musician to me is his over
200+ song discography (including unreleased tracks). I also love his
music and he has been my favorite musician for the past couple years as
well.

### How did you end up doing this analysis?

I joined the aforementioned facebook group back in January of 2019 as I
got into Alex's unreleased discography. Since then I have come to really
enjoy the community that exists there. One day an individual, Zion, who
asked how old people in the group were when they started listening to
Alex G and how old they were now. I asked if it was okay to collect the
data, and made a couple figures for the group that you will see
recreated in the analysis. I then asked if it was okay to make a survey
for the group to gather more data, I got the okay (specifically from
Isabelle) and began working. Many individuals helped with the survey as
you can see in the bottom, but the final dataset had `r ncol(dat)`
columns of data from `r round(nrow(dat), 3)` individuals. Some questions
Alex G related, some not. But I hope you find something interesting in
this little project I made. I've been working on it on and off in my
free time since late April and it has been in its final version since
late September with only small updates since. Either way, its a labor of
love to give back to the great community I found online. The group is
full of cool people and I'm glad to be a part of it (I also think this
project helped me become a ~~mod~~ admin so that was nice).
...
We'll look at age in a histogram colored on "what generation do you
identify with". Get used to the age variable, as it is one of our few
numerical values, so I'm going to be plotting it a lot. Anyways, overall
age is right-skewed, with only one real outlier at `r max(dat$age)`.
Alex G is about 26-27 at the time of me writing this and has been
writing music since his early teenage years, so the demographics of
...
over 200+ individuals and I came up with an idea of making two
...
looking at the rest ofThe Arts!
...
So... word clouds. Technically speaking word clouds are never a good way
to represent data in a meaningful way. While the size of the letters
...
phrases that are generated. Personally I love "Memory After Ur Gone",
it's just fun sticking his song titles together.
...
number 1 "liveness" song is Brick and Sugar House - LIVE, is third! We
also have loudness in decibels which goes from [Clouds (thanks
...
sad peep with some happy nerds around .75, breaking through the
...
bimodal distribution from danceability trends towards signifigance.
...
members come from the rarer types, and specifically, 18% of
participating members are the rarest type, INFJ, which makes up \~2% of
the normal population. Crazy. This, of course, carries over to the
...
In our group, when compared to the general population there does seem to
be an imbalance. Capricorns make up around 3% of our group while Gemini
makes up \~12%. That said, this is not a significant change in the
distribution according to a chi-square analysis. The same is true for
elements, the change nears significance, but does not cross the
arbitrary (thanks Fisher) 0.05 threshold, though overall there are less
Earth and Water signs in this group.
...
which I attribute to the trend that Slytherin's are genrally younger,
and this group a younger sample when compared to the overall populatio.

...
#race_plot
race_df <- as.data.frame(t(as.data.frame(table(dat$race))))
colnames(race_df) <- names(table(dat$race))
race_df <- race_df[2,]
rownames(race_df) <- "Individuals"

race_table <- ggtexttable(race_df)
  
  
ggarrange(age_plot, race_plot, race_df, nrow = 3)
```
...
gpt_plot <- ggarrange(gender_plot,
                      poly_plot, trans_plot,
                      nrow = 3, common.legend = T)


annotate_figure(gpt_plot, top = text_grob(
  "Gender count, polyamory colored on gender, and trans individuals"
))
...
Questions redo previous age analysis, use subtraction to gain more individuals based on previous data
```{r}
